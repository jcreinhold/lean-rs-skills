...
#   bash ${CLAUDE_PLUGIN_ROOT}/skills/module-design/scripts/audit-module.sh <path>
#
# Selection rules:
...
#                                                                 -> manual guidance (no Lean backend)
...
if is_lean "$TARGET"; then
	echo "Lean target detected. This skill ships no automated Lean backend."
	echo "Audit it by hand using references/lean4-patterns.md together with the"
	echo "lean-lsp MCP tools (lean_goal, lean_diagnostic_messages, lean_build)."
	exit 0
fi
